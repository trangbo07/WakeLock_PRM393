...
- **Đi bộ (`walk_task_page`):** phải đi đủ `walkMeters` mét (nhập lúc tạo).
  `WalkDetector` (`domain/walk_detector.dart`, pure/test): đếm bước từ gia tốc
  (peak > 11.8 m/s² sau khi dip về ~gravity), mét = bước × 0.75. Buộc đứng dậy
  đi/chạy; nằm yên không tăng. Đạt mét → auto pop success. Không cần
  quyền/thư viện mới (dùng lại `sensors_plus`).
...
- **Flashbang + rung (`AlarmEffects.kt`):** service gọi `startFlash()` (strobe
  đèn flash camera qua `CameraManager.setTorchMode`, ~350ms/lần, chỉ khi có
  FEATURE_CAMERA_FLASH) + `startVibration()` (rung dồn dập, on tăng/gap giảm dần).
  Bật theo `alarm.flashlight` / `alarm.vibrate`. Màn `AlarmRingingPage` cũng
  strobe nền amber↔đen khi `flashlight`. Emulator không có flash → chỉ thấy trên
  máy thật.
- **Chống chỉnh đồng hồ:** `AlarmRepository.rescheduleAllEnabled()` (re-anchor mọi
  alarm bật về `nextOccurrence` theo giờ hiện tại) gọi trong `app.dart` lúc start
  + mỗi resume. Đổi đồng hồ để né → lần mở app kế báo thức tự khớp lại giờ thực.
